...
def _configure_again(portal, origin: str) -> None:
    ...
    try:
        with contextlib.redirect_stdout(io.StringIO()):
            configure.ensure_app(_project(), portal, origin, TOKEN)
...
def test_the_door_service_is_held_under_the_identifier_the_step_chose():
    answer = registrations.get_user(configure.DOOR_SERVICE_ID, TOKEN)
    assert answer.status == 200, (
        f"nothing is held under the account identifier "
        f"{configure.DOOR_SERVICE_ID!r}: {answer.status} {answer.message()}")
    assert answer.body["user"]["username"] == configure.DOOR_SERVICE, answer.body
