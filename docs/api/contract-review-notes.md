...
The lint is clean. `npx @redocly/cli@2.49.0 lint docs/api/members-v1.yaml`
reports three warnings and no errors, which is the count and the identity ADR
0001 records: one `info-license`, two `operation-4xx-response` on the public
status endpoints. Those are not findings and are not repeated below.
...
the door controller. The contract's own `info.description` at line 58 says
...
- Authentication is applied consistently. `security` is declared globally at line
  85 and overridden to `[]` on exactly two operations, `/space_api.json` and
