...
- Six warnings are expected and are not defects, and this said five until
  2026-08-31. `info-license` fires because
  ...
  `no-unused-components` fires three times more, on the `RecordWasRemoved`,
  `NoSuchPath` and `WrongMethod`
  entries under `components.responses`. Those three are declared and referenced by
  ...
  earliest undeclared. `RecordWasRemoved` is unreferenced for a different reason,
  written where it is defined: the 409 on `POST /me` carries two slugs and
  OpenAPI allows one response per status, so that operation declares the pair
  inline. The `info.description` of `docs/api/members-v1.yaml` makes
