...
| `.github/workflows/ci.yml` and `tools/ci/` | **Built.** Thirteen jobs, twelve of which were green on a real runner on run 33228017933 on 2026-08-29, the run pull request 1 was merged on. The thirteenth, the bootstrap one, was added after that run and has not been through a runner yet. They run in parallel, so the wall clock is the slowest job and not the sum, and that is now the migration one. Measured on run 33228017933, the last before the merge: legacy import 58s, identity 55s, development stack 48s, database 20s, portal and mock 18s each, contract 11s, door 9s, prose and ceilings 7s each, commits 6s, theme 5s. The migration job took the lead from identity when it grew to eleven cases, each building its own database. Eight of the thirteen start containers, the ceilings one included, because it runs ruff as a pinned image rather than installing it. A separate workflow, the deploy, is dormant and runs only on `workflow_dispatch` with the hostname typed in |
...
the slowest of them at 58 seconds, though the bootstrap job added since that run has not been timed and starts a full identity stack.
