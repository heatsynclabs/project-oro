...
| `.github/workflows/ci.yml` and `tools/ci/` | **Built.** Fifteen jobs. Twelve were green on a real runner on run 33228017933 on 2026-08-29 and thirteen on run 33231216898 the same day. The backup and api jobs were added after those runs and have been green locally under `make check` but have not been through a runner yet. They run in parallel, so the wall clock is the slowest job and not the sum, and that is now the migration one. Measured on run 33228017933, the last before the merge: legacy import 58s, identity 55s, development stack 48s, database 20s, portal and mock 18s each, contract 11s, door 9s, prose and ceilings 7s each, commits 6s, theme 5s. The migration job took the lead from identity when it grew to eleven cases, each building its own database. Ten of the fifteen start containers, the ceilings one included, because it runs ruff as a pinned image rather than installing it. A separate workflow, the deploy, is dormant and runs only on `workflow_dispatch` with the hostname typed in |
